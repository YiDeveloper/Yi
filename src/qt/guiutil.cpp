...
{
    // Convert YiCoin:// to YiCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("YiCoin://"))
    {
        uri.replace(0, 12, "YiCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "YiCoin.lnk";
}
...
{
    return GetAutostartDir() / "YiCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("YiCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  YiCoin-qt [" + tr("command-line options") + "]                     " + "\n";
